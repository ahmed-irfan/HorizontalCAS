...
function getNetworks(folder="../networks_sxu/ACAS_sXu_v5_DNNs_rect",ver=5,epochs=200)
    ...
    for pra in ACTIONS
        nets_tau = []
        for tau in TAUS
            nnet_file_name = @sprintf("%s/ACAS_sXu_v%d_rect_model1_vertical_tau%d_pra%d_%dEpochs.nnet",folder,ver,tau,pra,epochs)
...
function reluValToMmap(folder,ver=5;praInds=1:NUMACTION,tauInds=1:NUMTAU)
...
function writeNetworkActionsMmap(;folder="NetworkApprox",nnetFolder="../networks_sxu/ACAS_sXu_v5_DNNs_rect",reluvalFolder="../ReluVal/Results",ver=5, epochs=200,useReluVal=false)
